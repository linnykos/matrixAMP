...
.generate_A <- function(n, epsilon, lambda){
  x_mat <- .generate_x(epsilon)
  x <- .sample_x(n, x_mat)
  (lambda/n)*x %*% t(x) + .generate_W(n)
}
...
.generate_x <- function(epsilon = 0.1){
  x_mat <- matrix(0, nrow = 2, ncol = 3)
  a <- sqrt(1/epsilon)
  x_mat[1,] <- c(-a, 0, a)
  x_mat[2,] <- c(epsilon/2, 1-epsilon, epsilon/2)
  
  rownames(x_mat) <- c("value", "probability")
  x_mat
}

.sample_x <- function(n, x_mat){
  stopifnot(all(dim(x_mat) == c(2,3)))
  sample(x_mat[1,], size = n, replace = T, prob = x_mat[2,])
}
